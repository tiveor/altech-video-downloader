...
A Chrome extension that detects and downloads videos from web pages, with full support for HLS (HTTP Live Streaming) to MP4 conversion.

![Chrome Extension](https://img.shields.io/badge/Manifest-V3-blue) ![Version](https://img.shields.io/badge/version-1.2.0-green)
...
- **Direct video downloads** — Supports MP4, WebM, OGG, AVI, MOV, MKV, FLV, WMV, and M4V formats
- **Master playlist handling** — Automatically resolves HLS master playlists and groups variant streams
- **DOM scanning** — Finds videos embedded in the page via `<video>` tags, `<source>` elements, `<a>` links, and Open Graph meta tags
...
2. **Content script** — A content script scans the DOM for `<video>` elements and observes mutations for dynamically loaded videos
3. **HLS processing** — HLS streams are downloaded via an offscreen document that fetches all `.ts` segments, defragments them using [mux.js](https://github.com/videojs/mux.js), and builds a proper MP4 container with correct `ftyp`, `moov`, and `mdat` atoms
...
1. Clone this repository:
   ```bash
   git clone https://github.com/alvarotech/altech-video-downloader.git
   ```
...
```bash
zip -r altech-video-downloader.zip \
